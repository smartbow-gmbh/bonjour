 ...
 * <P>
 * 
 * It is a factory class that is used to invoke registration and discovery-related operations. Most operations are non-blocking; clients are called back through
 * an interface with the result of an operation. Callbacks are made from a separate worker thread.
 * <P>
 * 
 ...
 * </PRE>
 * 
 * <CODE>MyClient.serviceFound()</CODE> would be called for every HTTP server discovered in the default browse domain(s).
 */

abstract public class DNSSD {
  /**
   * Flag indicates to a {@link BrowseListener} that another result is queued. Applications should not update their UI to display browse results if the
   * MORE_COMING flag is set; they will be called at least once more with the flag clear.
   */
  ...
  /**
   * If flag is set, allow multiple records with this name on the network (e.g. PTR records) when registering individual records on a {@link DNSSDRegistration}.
   */
   ...
   *          <P>
   * @param ifIndex
   *          If non-zero, specifies the interface on which to browse for services (the index for a given interface is determined via the if_nametoindex()
   *          family of calls.) Most applications will pass 0 to browse on all available interfaces. Pass -1 to only browse for services provided on the local
   *          host.
   *          <P>
   * @param regType
   *          The registration type being browsed for followed by the protocol, separated by a dot (e.g. "_ftp._tcp"). The transport protocol must be "_tcp" or
   *          "_udp".
   *          <P>
   * @param domain
   *          If non-null, specifies the domain on which to browse for services. Most applications will not specify a domain, instead browsing on the default
   *          domain(s).
   *          <P>
   * @param listener
   ...
   *           If a security manager is present and denies <tt>RuntimePermission("getDNSSDInstance")</tt>.
   ...
   * <P>
   * 
   * Note: resolve() behaves correctly for typical services that have a single SRV record and a single TXT record (the TXT record may be empty.) To resolve
   * non-standard services with multiple SRV or TXT records, use queryRecord().
   * <P>
   * 
   * @param flags
   *          Currently ignored, reserved for future use.
   *          <P>
   * @param ifIndex
   *          The interface on which to resolve the service. The client should pass the interface on which the serviceName was discovered (i.e. the ifIndex
   ...
   *          <P>
   * @param regType
   *          The registration type being resolved followed by the protocol, separated by a dot (e.g. "_ftp._tcp"). The transport protocol must be "_tcp" or
   *          "_udp".
   *          <P>
   * @param domain
   *          The domain on which the service is registered, i.e. the domain passed to the serviceFound() callback.
   *          <P>
   * @param listener
   ...
   * @see RuntimePermission
   */
  public static DNSSDService resolve(int flags, int ifIndex, String serviceName, String regType, String domain, ResolveListener listener)
   ...
   *          <P>
   * @param ifIndex
   *          If non-zero, specifies the interface on which to register the service (the index for a given interface is determined via the if_nametoindex()
   *          family of calls.) Most applications will pass 0 to register on all available interfaces. Pass -1 to register a service only on the local machine
   *          (service will not be visible to remote hosts).
   *          <P>
   * @param serviceName
   *          If non-null, specifies the service name to be registered. Applications need not specify a name, in which case the computer name is used (this name
   *          is communicated to the client via the serviceRegistered() callback).
   *          <P>
   * @param regType
   *          The registration type being registered followed by the protocol, separated by a dot (e.g. "_ftp._tcp"). The transport protocol must be "_tcp" or
   *          "_udp".
   *          <P>
   * @param domain
   *          If non-null, specifies the domain on which to advertise the service. Most applications will not specify a domain, instead automatically
   *          registering in the default domain(s).
   *          <P>
   * @param host
   *          If non-null, specifies the SRV target host name. Most applications will not specify a host, instead automatically using the machine's default host
   *          name(s). Note that specifying a non-null host does NOT create an address record for that host - the application is responsible for ensuring that
   *          the appropriate address record exists, or creating it via {@link DNSSDRegistration#addRecord}.
   *          <P>
   * @param port
   *          The port on which the service accepts connections. Pass 0 for a "placeholder" service (i.e. a service that will not be discovered by browsing, but
   *          will cause a name conflict if another client tries to register that same name.) Most clients will not use placeholder services.
   *          <P>
   * @param txtRecord
   *          The txt record rdata. May be null. Note that a non-null txtRecord MUST be a properly formatted DNS TXT record, i.e. &lt;length byte&gt;
   ...
   * @see RuntimePermission
   */
  public static DNSSDRegistration register(int flags, int ifIndex, String serviceName, String regType, String domain, String host, int port,
      TXTRecord txtRecord, RegisterListener listener) throws DNSSDException {
   ...
   *          <P>
   * @param ifIndex
   *          If non-zero, specifies the interface on which to issue the query (the index for a given interface is determined via the if_nametoindex() family of
   *          calls.) Passing 0 causes the name to be queried for on all interfaces. Passing -1 causes the name to be queried for only on the local host.
   *          <P>
   * @param serviceName
   ...
   *          <P>
   * @param ifIndex
   *          If non-zero, specifies the interface on which to look for domains. (the index for a given interface is determined via the if_nametoindex() family
   *          of calls.) Most applications will pass 0 to enumerate domains on all interfaces.
   *          <P>
   * @param listener
  ...
  /**
   * Concatenate a three-part domain name (as provided to the listeners) into a properly-escaped full domain name. Note that strings passed to listeners are
   * ALREADY ESCAPED where necessary.
   * <P>
   * 
   * @param serviceName
   *          The service name - any dots or slashes must NOT be escaped. May be null (to construct a PTR record name, e.g. "_ftp._tcp.apple.com").
   *          <P>
   * @param regType
   ...
   *          <P>
   * @param ifIndex
   *          If non-zero, specifies the interface on which to reconfirm the record (the index for a given interface is determined via the if_nametoindex()
   *          family of calls.) Passing 0 causes the name to be reconfirmed on all interfaces. Passing -1 causes the name to be reconfirmed only on the local
   *          host.
   *          <P>
   * @param fullName
  ...
  abstract protected DNSSDService _makeBrowser(int flags, int ifIndex, String regType, String domain, BrowseListener listener) throws DNSSDException;

  abstract protected DNSSDService _resolve(int flags, int ifIndex, String serviceName, String regType, String domain, ResolveListener listener)
      throws DNSSDException;

  abstract protected DNSSDRegistration _register(int flags, int ifIndex, String serviceName, String regType, String domain, String host, int port,
      TXTRecord txtRecord, RegisterListener listener) throws DNSSDException;

  abstract protected DNSSDRecordRegistrar _createRecordRegistrar(RegisterRecordListener listener) throws DNSSDException;

  abstract protected DNSSDService _queryRecord(int flags, int ifIndex, String serviceName, int rrtype, int rrclass, QueryListener listener)
      throws DNSSDException;

  abstract protected DNSSDService _enumerateDomains(int flags, int ifIndex, DomainListener listener) throws DNSSDException;

  abstract protected String _constructFullName(String serviceName, String regType, String domain) throws DNSSDException;

  abstract protected void _reconfirmRecord(int flags, int ifIndex, String fullName, int rrtype, int rrclass, byte[] rdata);
...
class AppleDNSSDException extends DNSSDException {
  ...
  public String getMessage() {
    final String kMessages[] = { // should probably be put into a resource or something
        "UNKNOWN", "NO_SUCH_NAME", "NO_MEMORY", "BAD_PARAM", "BAD_REFERENCE", "BAD_STATE", "BAD_FLAGS", "UNSUPPORTED", "NOT_INITIALIZED", "NO_CACHE",
        "ALREADY_REGISTERED", "NAME_CONFLICT", "INVALID", "FIREWALL", "INCOMPATIBLE", "BAD_INTERFACE_INDEX", "REFUSED", "NOSUCHRECORD", "NOAUTH", "NOSUCHKEY",
        "NATTRAVERSAL", "DOUBLENAT", "BADTIME", "BADSIG", "BADKEY", "TRANSIENT", "SERVICENOTRUNNING", "NATPORTMAPPINGUNSUPPORTED", "NATPORTMAPPINGDISABLED" };
...
class AppleDNSSD extends DNSSD {
  static {
    ...
    try {
      System.load(DNSSDDllProvider.getDLL("dnssd"));

      System.load(DNSSDDllProvider.getDLL("jdns_sd"));
    } catch (IOException e) {
      throw new InternalError("Could not load native libraries ", e);
    }
  ...
  protected DNSSDRegistration _register(int flags, int ifIndex, String serviceName, String regType, String domain, String host, int port, TXTRecord txtRecord,
      RegisterListener client) throws DNSSDException {
  ...
  protected native void ReconfirmRecord(int flags, int ifIndex, String fullName, int rrtype, int rrclass, byte[] rdata);
...
class AppleRegistration extends AppleService implements DNSSDRegistration {
  public AppleRegistration(int flags, int ifIndex, String serviceName, String regType, String domain, String host, int port, byte[] txtRecord,
      RegisterListener client) throws DNSSDException {
    super(client);
